...
import org.appxi.smartlib.util.Period;
...
class SearcherController extends WorkbenchMainViewController {
    ...
    private ListView<FacetFilter> facetCatalogView, facetPeriodView, facetLibraryView, facetAuthorView;
    ...
    protected void onViewportInitOnce(StackPane viewport) {
        ...
        Tab tab3 = new Tab("库", facetLibraryView = new ListViewEx<>(this::handleEventOnFacetEnterOrDoubleClick));
        ...
        facetLibraryView.setCellFactory(facetCellFactory);
    ...
    private void handleEventOnFacetEnterOrDoubleClick(InputEvent inputEvent, FacetFilter item) {
        ...
        selectedList.addAll(getSelectedFacetFilters(facetLibraryView));
    ...
    SearcherController setSearchScope(Item searchScope) {
        this.searchScope.clear();
        if (null != searchScope) this.searchScope.add(searchScope.getPath());
        FxHelper.runLater(() -> {
            this.searchScopeTipLabel.setText(null != searchScope ?
                    "在 ".concat(searchScope.typedPath()).concat(" 中搜索") : "在 全局 中搜索");
            if (null != inputView)
    ...
    private void handleSearching(String inputText) {
        setTitles(inputText);
        // 使用线程，避免UI阻塞假死
        blockingHandler = ProgressLayer.show(getViewport(), progressLayer -> handleSearchingImpl(inputText));
    }
    ...
    private void handleSearchingImpl(String inputText) {
        ...
        if (inputText.equals(searchedText) && null != facetAndHighlightPage && facetAndHighlightPage.getTotalElements() > 0) {
            ...
            categories.addAll(getSelectedFacetFilterValues(facetLibraryView));
            categories.addAll(getSelectedFacetFilterValues(facetAuthorView));
        }
        searchedText = inputText;
        inputQuery = inputText;
        facetAndHighlightPage = repository.search(searchScope, searchTypes,
        ...
        // update facet ui
        final List<FacetFilter> facetLibraryList = new ArrayList<>(), facetCatalogList = new ArrayList<>(),
                facetPeriodList = new ArrayList<>(), facetAuthorList = new ArrayList<>();
        if (facet) {
            final Map<String, List<FacetFilter>> listMap = Map.of(
                    "/library/", facetLibraryList,
            ...
            facetLibraryList.sort(Comparator.comparing(v -> String.valueOf(v.order)));
        ...
        Runnable runnable = () -> {
            if (finalFacet) {
                ...
                facetLibraryView.getItems().setAll(facetLibraryList);
    ...
    private Node createResultPageView(int pageIdx) {
        ...
        if (pageIdx == 0) {
            ...
            // query for next page
            highlightPage = repository.search(searchScope, searchTypes,
                    inputQuery, categories, false, new SolrPageRequest(pageIdx, PAGE_SIZE));
        }
        if (null == highlightPage)
            return new Label();// avoid NPE

        resultPageView = new ListViewEx<>((event, item) -> app.eventBus.fireEvent(new SearchedEvent(item, inputQuery, null)));
                ...
                if (!highlights.isEmpty()) {
                    highlights.forEach(highlight -> {
                        highlight.getSnipplets().forEach(str -> {
                            String[] strings = "…".concat(str).concat("…").split("§§hl#pre§§");
